...
if snakemake.params.mark_duplicates == True:
    ...
    if snakemake.params.UMI == "no_umi" or snakemake.params.umi_usage == "no":
        ...
        + " --extract-umi-method=read_id --umi-separator='" + snakemake.params.umi_sep + "' --method=directional --edit-distance-threshold=0 --spliced-is-unique --multimapping-detection-method=NH"
        f = open(log_filename, 'at')
        f.write("## COMMAND: "+command+"\n")
        f.close()
        shell(command)
